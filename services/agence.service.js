...
module.exports={ 
    ...
    search_transact: async (data)=>{
        ...
        if(data.type_operation == 'Depot'){
            const [transac, metadata] = await sequelize.query("SELECT DATE(depots.createdAt) dat,depots.id_user id_user,depots.montant_depose mont,depots.pourc_preleve pourc,depots.devise FROM depots WHERE DATE(depots.createdAt)=:date_",
            {
            ...
            if(data.type_operation == 'Retrait'){
                const [transac, metadata] = await sequelize.query("SELECT DATE(retraits.createdAt) dat,retraits.id_user id_user,retraits.montant_retire mont,retraits.pourc_preleve pourc,retraits.devise FROM retraits WHERE DATE(retraits.createdAt)=:date_",
                {
                    replacements: { date_:data.date_}
                }); 
                transac_ = JSON.parse(JSON.stringify(transac)) 
                ...
                if(data.type_operation == 'Transfert'){
                    const [transac, metadata] = await sequelize.query("SELECT DATE(transferts.createdAt) dat,transferts.id_user id_user,transferts.montant_envoye mont,transferts.pourc_preleve pourc,transferts.devise FROM transferts WHERE DATE(transferts.createdAt)=:date_",
                    {
                        replacements: { date_:data.date_}
                    }); 
                    transac_ = JSON.parse(JSON.stringify(transac)) 
                            ...
                            if(await this.solde_membre(compte_rec) =='cmt_null' ) msg = 4 // Vérification existence compte recipiendaire
                                ...
                                if(parseFloat(await this.solde_membre(compte_exp)) > parseFloat(montant_plus_frais)){ // Vérification solde
                                    let trans = await Transfer.create({
                                        ...
                                        ref_operation    : this.generateRefOperation(),
                                        id_user          : user,
                                        devise           : data.devise,
                                        annule           : 0,
                                    ...
                                    /* OTP Expéditeur */
                                        ...
                                        let numPhone=id_compte_exp[0].telephone,
                                            message=`Transfert effectué sur          
                                        ${id_compte_rec[0].prenom+' '+id_compte_rec[0].nom}        
                                        Montant : ${trans.dataValues.montant_envoye + ' '+data.devise}    
                                        Compte:  ${id_compte_rec[0].compte}
                                        Frais: ${trans.dataValues.pourc_preleve+ ' '+data.devise}
                                        Solde : ${await this.solde_membre(compte_exp) +' '+trans.dataValues.devise}
                                        Réf : ${trans.dataValues.ref_operation}
                                        Merci.`,
                                    ...
                                    /* OTP Recepteur */
                                        ...
                                        let numPhone2=id_compte_rec[0].telephone,
                                            message2=`Vous avez reçu de        
                                        ${id_compte_exp[0].prenom+' '+id_compte_exp[0].nom}        
                                        ${trans.dataValues.montant_envoye + ' '+data.devise}    
                                        Compte:  ${id_compte_exp[0].compte}
                                        Solde : ${await this.solde_membre(compte_rec) +' '+trans.dataValues.devise}
                                        Réf : ${trans.dataValues.ref_operation}
                                        Merci.`,
                        ...
                        if(parseFloat(montant_min[0].f) <= parseFloat(data.montant)){
                            ...
                            if(parseFloat(await this.solde_membre(data)) > parseFloat(montant_plus_frais)){
                                ...
                                if(retrait.dataValues.ref_operation){
                                    let numPhone=id_compte[0].telephone,
                                        message =`Retrait effectué sur        
                                        ${' '+id_compte[0].prenom+ ' '+id_compte[0].nom}  
                                        Compte : ${id_compte[0].compte}        
                                        Montant : ${retrait.dataValues.montant_retire+ ' '+data.devise}      
                                        Frais : ${parseFloat((retrait.dataValues.montant_retire * retrait.dataValues.pourc_preleve) / 100).toFixed(2)+ ' '+data.devise}  
                                        Solde : ${await this.solde_membre(data)+ ' '+data.devise}       
                                        Réf : ${retrait.dataValues.ref_operation}       
                                        Merci.`,
                                        url = `https://api2.dream-digital.info/api/SendSMS?api_id=API42124386641&api_password=x3McB19tzd&sms_type=T&encoding=T&sender_id=LoanMe CD&phonenumber=${numPhone}&textmessage=${message}`;
                                    await axios.get(url);
                                    msg='r'
                ...
                if(parseFloat(montant_min[0].f) <= parseFloat(data.montant)){
                    if((await this.solde_monnaie_electronique_agence(agence,data.devise)  >= parseFloat(data.montant))){ // vérification solde
                        ...
                        if(depot.dataValues.ref_operation){
                            let numPhone=id_compte[0].telephone,
                                message =`Dépôt effectué sur        
                                ${' '+id_compte[0].prenom+ ' '+id_compte[0].nom}  
                                Compte : ${id_compte[0].compte}        
                                Montant : ${depot.dataValues.montant_depose+ ' '+data.devise}       
                                Solde : ${await this.solde_membre(data)+ ' '+data.devise}       
                                Réf : ${depot.dataValues.ref_operation}       
                                Merci.`,
                                url = `https://api2.dream-digital.info/api/SendSMS?api_id=API42124386641&api_password=x3McB19tzd&sms_type=T&encoding=T&sender_id=LoanMe CD&phonenumber=${numPhone}&textmessage=${message}`;
                            await axios.get(url);
                            msg='r'
