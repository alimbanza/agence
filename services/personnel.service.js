const {sequelize,Op,User,Employe,Membre,Partenaire_membre} = require('../config/db');
const fs = require('fs')
...
module.exports={
    ...
    enregistrer_personnel : async function (data,agence,file){
        try {
            let ps              = '', 
                mdp_user        = "1234",
                        ...
                        if(await this.check_type_piece_and_num_piece(data) == ''){
                            if(data.type_membre == 6){
                                ...
                                /*  if(await this.check_pseudo_employe(data.prenom)==''){
                                    ...
                                    let employ = await Employe.create({
                                        id_user           :user.dataValues.id,
                                        id_agence         :agence,
                                        id_partenaire     :1,
    ...
    rep_personnel : async (dataMembre,agence)=>{
        let pers = ''
        if(dataMembre.type == 1){
            if(dataMembre.id == 6){
                ...
                const [pers_, meta] = await sequelize.query("SELECT users.*,agences.denomination,type_employes.libelle,type_employes.id id_type FROM users INNER JOIN employes ON employes.id_user=users.id INNER JOIN agences ON agences.id = employes.id_agence INNER JOIN type_employes ON type_employes.id=employes.id_type_employe WHERE agences.id=:id_agence AND type_employes.id=:type_membre",
                {
                    replacements: { type_membre:dataMembre.id,id_agence:agence }
                })
                pers = JSON.parse(JSON.stringify(pers_))
            }
        }else{
            const [pers_, meta] = await sequelize.query("SELECT users.*,agences.denomination FROM users INNER JOIN structure_users ON structure_users.id_user=users.id INNER JOIN agences ON agences.id = structure_users.id_agence WHERE agences.id=:id_agence AND users.id=:id AND users.id_type<>1 AND users.id_type<>7",
            {
                replacements: { id:dataMembre.id,id_agence:agence }
            })
            pers = JSON.parse(JSON.stringify(pers_))
        }
        return pers
    ...
    membre_actif : async (id_user)=>{ 
        let on_personne_ = ''
        const [on_personne, meta] = await sequelize.query("SELECT users.*,partenaires.id id_part,partenaire_membres.etat_user etat_us,partenaires.denomination,membres.id_type_membre id_type FROM users INNER JOIN membres ON membres.id_user=users.id INNER JOIN partenaire_membres ON partenaire_membres.id_membre=membres.id INNER JOIN partenaires ON partenaires.id=partenaire_membres.id_partenaire WHERE users.id=:id AND partenaire_membres.etat_user=1",
        {
            replacements: { id:id_user }
        })
        on_personne_ = JSON.parse(JSON.stringify(on_personne))
        if(on_personne_ == ''){
            const [on_pers, meta] = await sequelize.query("SELECT users.* FROM users INNER JOIN employes ON employes.id_user=users.id WHERE users.id=:id AND employes.etat_user=1",
            {
                replacements: { id:id_user }
            })
            on_personne_ = JSON.parse(JSON.stringify(on_pers))
        }
        return on_personne_
    },
